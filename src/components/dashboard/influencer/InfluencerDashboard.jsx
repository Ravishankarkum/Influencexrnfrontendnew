...
// Mapping API icon names to actual lucide-react icon components
const iconMap = {
  DollarSign,
  MessageSquare,
  Clock,
...
export function InfluencerDashboard() {
  ...
  useEffect(() => {
    const fetchDashboard = async () => {
      ...
      try {
        ...
        // Fetch influencer dashboard data through apiService
        const data = await apiService.dashboard.getInfluencerDashboard();

        // Set different dashboard sections
        setStats(data?.stats || []);
  ...
  // Return Tailwind color classes based on campaign status
    ...
    <div className="space-y-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((stat, index) => {
          const Icon = iconMap[stat.icon]; // Pick correct icon from map
          return (
            <div
              key={index}
              className="bg-gradient-to-tr from-indigo-100 to-indigo-200 p-5 rounded-2xl shadow hover:scale-[1.02] transition-transform duration-200 border"
            >
              <div className="flex items-center justify-between mb-3">
                <div className="p-3 rounded-full bg-indigo-500 text-white shadow-lg">
                  {Icon && <Icon size={24} />} {/* Render icon only if valid */}
                </div>
              </div>
